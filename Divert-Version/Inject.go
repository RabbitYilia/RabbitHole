...
func SendText(Handle uintptr, input string) {
	md5Ctx := md5.New()
	...
	for thispiece, piecedmsg := range SplitedMsgs {
		...
		if strings.Contains(DstIP.String(), ".") {
			...
			gopacket.SerializeLayers(buffer, options, ipv4Layer, UDPLayer, gopacket.Payload(TXJson))
		} else {
			...
			gopacket.SerializeLayers(buffer, options, ipv6Layer, UDPLayer, gopacket.Payload(TXJson))
		}
		SendOut(Handle, buffer.Bytes())
	}
}
