...
var IPv6Map []net.IP
var IPv4Map []net.IP
var Peerv6Map []net.IP
var Peerv4Map []net.IP
	...
	if ThisRXPacket.ApplicationLayer() != nil {
		...
		if err == nil {
			...
			if TTL == 1 {
				...
				if TTL == 2 {
					DstIP = net.ParseIP(RXdata["DstIP"])
					SrcIP = net.ParseIP(RXdata["SrcIP"])
				} else {
					if strings.Contains(RXdata["DstIP"], ".") {
						DstIP = Peerv4Map[RandInt(0, len(Peerv4Map)-1)]
						SrcIP = IPv4Map[RandInt(0, len(IPv4Map)-1)]
					} else {
						DstIP = Peerv6Map[RandInt(0, len(Peerv6Map)-1)]
						SrcIP = IPv6Map[RandInt(0, len(IPv6Map)-1)]
					}
				}
